"""Freshness analyzer standalone skill script."""
...
try:
    ...
    from member2.audits.freshness_audit import audit_freshness


def main():
    if len(sys.argv) < 2:
        print("Usage: python freshness.py <snapshot.json>")
        sys.exit(1)
    ...
    findings = audit_freshness(snapshot, parsed)
